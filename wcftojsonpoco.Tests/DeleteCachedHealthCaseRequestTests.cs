...
using eMedicalService.LegacyJavaWcfService.DeleteCachedHealthCase;
using Xunit;
using Xunit.Abstractions;

namespace wcftojsonpoco.Tests;

public class DeleteCachedHealthCaseRequestTests
{
    ...
    public DeleteCachedHealthCaseRequestTests(ITestOutputHelper output)
    {
        _output = output;
    }
    ...
    {
        // Arrange
        var jsonPath = Path.Combine(AppContext.BaseDirectory, "Domain.Contracts.Messages.deleteCachedHealthCaseRequestType.json");
        var originalJsonContent = File.ReadAllText(jsonPath);

        // Act - Deserialize the original JSON
        var deserialized = JsonSerializer.Deserialize<deleteCachedHealthCaseRequestType>(originalJsonContent, _options);
        ...
        var originalOutputPath = Path.Combine(outputDir, "original_deleteCachedHealthCase.json");
        var reserializedOutputPath = Path.Combine(outputDir, "reserialized_deleteCachedHealthCase.json");
        ...
        sb.AppendLine("=== COMPREHENSIVE JSON FIELD COMPARISON (DeleteCachedHealthCaseRequest) ===\n");
        ...
        // Write to file
        var comparisonOutputPath = Path.Combine(outputDir, "field_comparison_report_deleteCachedHealthCase.txt");
